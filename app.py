...
STATUS_META = {
    "PENDENTE": {
        "label": "Verificação de disponibilidade",
        "class": "pendente",
        "description": "Estamos verificando se o item está disponível no almoxarifado.",
    },
    "VERIFICANDO_DISPONIBILIDADE": {
        "label": "Verificação de disponibilidade",
        "class": "pendente",
        "description": "O almoxarifado está verificando se possui o item em estoque.",
    },
    "LIBERADO_RETIRADA": {
        "label": "Produto liberado",
        "class": "aprovado",
        "description": "O item está disponível e pode ser retirado no almoxarifado.",
    },
    "EM_PROCESSO_COMPRA": {
        "label": "Em processo de compra",
        "class": "em-compra",
        "description": "O item não está disponível em estoque e já foi direcionado para compras.",
    },
    "APROVADO": {
        "label": "Aprovado",
        "class": "aprovado",
...
def status_meta(status: str | None):
    normalized = (status or "PENDENTE").strip().upper()
    return STATUS_META.get(normalized, STATUS_META["PENDENTE"])


def enrich_request_row(row):
    data = dict(row)
    meta = status_meta(data.get("status"))
    data["status_label"] = meta["label"]
    data["status_class"] = meta["class"]
    data["status_description"] = meta["description"]
    return data
...
def init_db():
    ...
    ensure_column(conn, "requests", "admin_observacao", "admin_observacao TEXT")
...
def requester_home():
    conn = get_conn()
    requester_requests = conn.execute(
        """
        SELECT *
        FROM requests
        WHERE solicitante_nome = ?
          AND COALESCE(solicitante_matricula, '') = ?
        ORDER BY id DESC
        """,
        (session.get("requester_nome", ""), session.get("requester_matricula", "")),
    ).fetchall()
    conn.close()

    requests_view = [enrich_request_row(r) for r in requester_requests]
    return render_template("index.html", requester_requests=requests_view)
...
def solicitar_catalogo():
    ...
    conn.execute(
        ...
        INSERT INTO requests (
            ...
            foto_referencia, product_id, status, data_criacao, data_atualizacao, nome_tecnico
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'VERIFICANDO_DISPONIBILIDADE', ?, ?, ?)
        """,
        (
            "CATALOGO",
            requester["nome"],
...
def solicitar_item_novo():
    ...
    if not item_nome or not onde_sera_usado or not nome_tecnico:
        flash("Informe o nome do item, o nome técnico e onde ele será usado.", "erro")
        return redirect(url_for("requester_home"))
...
def admin_dashboard():
    ...
    pending_statuses = ("PENDENTE", "VERIFICANDO_DISPONIBILIDADE")
    approved_statuses = ("APROVADO", "LIBERADO_RETIRADA")

    total_requests = conn.execute("SELECT COUNT(*) AS total FROM requests").fetchone()["total"]
    total_pending = conn.execute(
        "SELECT COUNT(*) AS total FROM requests WHERE status IN (?, ?)",
        pending_statuses,
    ).fetchone()["total"]
    ...
    urgent_open = conn.execute(
        "SELECT COUNT(*) AS total FROM requests WHERE status IN (?, ?) AND prioridade = 'Urgente'",
        pending_statuses,
    ).fetchone()["total"]
    in_purchase = conn.execute("SELECT COUNT(*) AS total FROM requests WHERE status = 'EM_PROCESSO_COMPRA'").fetchone()["total"]

    latest_requests = [enrich_request_row(r) for r in conn.execute(
    ...
    sector_kpis = conn.execute(
        ...
        SELECT
            ...
            COUNT(*) AS total,
            SUM(CASE WHEN status IN ('PENDENTE', 'VERIFICANDO_DISPONIBILIDADE') THEN 1 ELSE 0 END) AS pendentes,
            SUM(CASE WHEN status IN ('APROVADO', 'LIBERADO_RETIRADA') THEN 1 ELSE 0 END) AS aprovadas,
            ...
            ROUND((SUM(CASE WHEN status IN ('APROVADO', 'LIBERADO_RETIRADA') THEN 1 ELSE 0 END) * 100.0) / COUNT(*), 1) AS taxa_aprovacao
        FROM requests
...
def admin_requests():
    ...
    allowed_statuses = {
        "PENDENTE", "APROVADO", "RECUSADO", "VERIFICANDO_DISPONIBILIDADE", "LIBERADO_RETIRADA", "EM_PROCESSO_COMPRA"
    }
    ...
    conn = get_conn()
    requests_data = [enrich_request_row(r) for r in conn.execute(query, params).fetchall()]
    conn.close()
...
def update_request_status(request_id, new_status):
    ...
    allowed_statuses = {"PENDENTE", "APROVADO", "RECUSADO", "VERIFICANDO_DISPONIBILIDADE", "LIBERADO_RETIRADA", "EM_PROCESSO_COMPRA"}
    ...
    flash(f"Solicitação atualizada para {status_meta(new_status)['label']}.", "sucesso")
    return redirect(url_for("admin_requests"))
